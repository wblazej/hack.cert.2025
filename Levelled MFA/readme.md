...
It's basically the same challenge as Easy MFA, but it uses LGC instead of standard python random. Now we get 64 passwords and it's enough to predict the state using [this attack](https://github.com/jvdsn/crypto-attacks/blob/master/attacks/lcg/truncated_state_recovery.py) I found. Then everything's the same - predict the secret key, generate admin's token, get last OTP, predict OTP codes until it matches the last one, and send the next OTP. Entire solution:
```py
